...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // ICS Import/Export routes
  app.post('/api/calendar/export/:professorId', async (req, res) => {
    try {
      ...
      const { generateProfessorICS } = await import('./ics-service');
      
      const icsContent = await generateProfessorICS(
        professorId,
        new Date(startDate),
        new Date(endDate)
      );

      // Buscar nome do professor para o filename
      const professor = await db.select()
        .from(users)
        .where(eq(users.id, professorId))
        .limit(1);

      const professorName = professor[0]?.name || professor[0]?.username || 'professor';
      const filename = `agenda-${professorName.toLowerCase().replace(/\s+/g, '-')}.ics`;
      ...
      res.status(500).json({ 
        error: 'Erro ao gerar arquivo de calendário',
  ...
  app.post('/api/calendar/analyze', async (req, res) => {
    try {
      const { icsContent, professorId } = req.body;

      if (!icsContent || !professorId) {
        return res.status(400).json({ 
          error: 'icsContent e professorId são obrigatórios' 
        });
      }

      const { analyzeICSFile } = await import('./ics-service');
      
      const analysis = await analyzeICSFile(icsContent, parseInt(professorId));
      ...
      res.status(500).json({ 
        error: 'Erro ao analisar arquivo de calendário',
        details: error.message 
      });
    }
  });
  ...
  app.post('/api/calendar/import', async (req, res) => {
    try {
      ...
      if (!eventsToImport || !professorId || !defaultStudentId) {
        return res.status(400).json({ 
          error: 'eventsToImport, professorId e defaultStudentId são obrigatórios' 
        });
      }

      // Se há eventos recorrentes e não foi confirmado, retornar para confirmação
      const hasRecurringEvents = eventsToImport.some((event: any) => event.rrule);
      
      if (hasRecurringEvents && !confirmRecurring) {
        return res.status(200).json({
          requiresConfirmation: true,
          message: 'Foram encontrados eventos recorrentes. Confirme a importação.',
          recurringCount: eventsToImport.filter((event: any) => event.rrule).length
        });
      }

      const { importICSEvents } = await import('./ics-service');
      
      const result = await importICSEvents(
        eventsToImport,
        parseInt(professorId),
        studentMapping || {},
        parseInt(defaultStudentId)
      );
      ...
      res.status(500).json({ 
        error: 'Erro ao importar eventos do calendário',
        details: error.message 
      });
    }
  });
